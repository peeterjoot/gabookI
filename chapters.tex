%
% Copyright © 2013 Peeter Joot.  All Rights Reserved.
% Licenced as described in the file LICENSE under the root directory of this GIT repository.
%
\part{Basics and Geometry}
%   % have lots of intros here.  Sort them all out.
%   \input{basics/introGa.tex}
%   \input{basics/gaQuickIntro.tex}
%   \input{basics/gaGradeDotWedge.tex}

   \chapter{Basics}
      \section{Did you ever ask your teacher how to multiply vectors?}
         \input{../GAelectrodynamics/.junk/GAmotivation.tex}
         \subsection{Problems}
            \input{../GAelectrodynamics/.junk/ComplexInnerProductVsDotAndCrossProduct.tex}
      \section{Vector multiplication}
         \input{../GAelectrodynamics/multiplication.tex}
         \subsection{Problems}
            \input{../GAelectrodynamics/.junk/2dvectorsquare.tex}
            \input{../GAelectrodynamics/.junk/normalAnticommutation.tex}
      \section{Definitions}
         \input{../GAelectrodynamics/.junk/definitions.tex}
         \subsection{Problems}
            \input{../GAelectrodynamics/R3PseudoscalarSquare.tex}
      \section{Grade selection, dot and wedge product operators}
         \input{../GAelectrodynamics/.junk/gradeselection.tex}
         \subsection{Problems}
            \input{../GAelectrodynamics/.junk/RnDotProduct.tex}
            \input{../GAelectrodynamics/working/cyclicpermutationtwo.tex}
            \input{../GAelectrodynamics/.junk/dotprodSymmetricSum.tex}
            \input{../GAelectrodynamics/.junk/planeRotationsExponentials.tex}
            \input{../GAelectrodynamics/.junk/complexNumbers.tex}
            \input{../GAelectrodynamics/.junk/R3PseudoscalarCommutation.tex}
            \input{../GAelectrodynamics/.junk/gradeselVectorWedge.tex}
            \input{../GAelectrodynamics/.junk/WedgeRelationshipToCrossProduct.tex}
            \input{../GAelectrodynamics/.junk/vectorBivectorDot.tex}
            \input{../GAelectrodynamics/vectorTrivectorDot.tex}
            \input{../GAelectrodynamics/.junk/bivectorDot.tex}
            \input{../GAelectrodynamics/r4nonzerobivectorwedgewithself.tex}
            \input{../GAelectrodynamics/working/scalarPermutation.tex}
      \section{Product of two vectors}
         \input{../GAelectrodynamics/.junk/vectorproduct.tex}
         \subsection{Miscellanious theorems}
            \input{../stokesTheorem/bladeDotWedgeSymmetryIdentitiesTheorem.tex}
         %   \input{appendix/wedgeDistributionIdentity.tex}
         \subsection{Problems}
            \input{../GAelectrodynamics/.junk/vectorproductCyclicPermutation.tex}
            \input{../GAelectrodynamics/.junk/wedgeantisym.tex}
            \input{../GAelectrodynamics/.junk/gradethreeselectionWedge.tex}
            \input{../stokesTheorem/bladeDotWedgeSymmetryIdentities.tex}
            \input{appendix/wedgeDistributionIdentityProblems.tex}
%      \section{Problem solutions}
%         \shipoutAnswer

   \chapter{Comparison of many traditional vector and GA identities}
      \input{basics/gaWiki.tex}
   \chapter{Cramer's rule}
      \input{basics/gaWikiCramers.tex}
   \chapter{Torque}
      \input{basics/gaWikiTorque.tex}
   \chapter{Derivatives of a unit vector}\label{chap:PJUnitDer}
      \input{basics/gaWikiUnitDerivative.tex}
   \chapter{Radial components of vector derivatives}\label{chap:PJRadialDer}
      \input{basics/radialVectorDerivatives.tex}
   \chapter{Rotational dynamics}\label{chap:PJAngVel}
      \input{basics/angularVelocity.tex}
   \chapter{Bivector Geometry}
      \input{basics/bivector.tex}
   \chapter{Trivector geometry}
      \input{basics/trivector.tex}
   \chapter{Multivector product grade zero terms}
      \input{basics/scalarCommutes.tex}
   \chapter{Blade grade reduction}
      \input{basics/bladegradereduction.tex}
   \chapter{More details on NFCM plane formulation}
      \input{basics/plane.tex}
   \chapter{Quaternions}
      \input{basics/quaternion.tex}
   \chapter{Cauchy Equations expressed as a gradient}
      \input{basics/cauchyGradient.tex}
   \chapter{Legendre Polynomials}
      \input{basics/legendre.tex}
   \chapter{Levi-Civitica summation identity}
      \input{basics/levi.tex}
   \chapter{Some NFCM exercise solutions and notes}
      \input{basics/nfcmCh2.tex}
   \chapter{Outermorphism Question}
      \input{basics/outermorphismDet.tex}
   %%\chapter{Elliptic parameterization}
      \input{basics/hestenesElipseParameterization.tex}

\part{Projection}
   \chapter{Reciprocal Frame Vectors}
      \input{projection/reciprocalFrame.tex}
   \chapter{Matrix review}\label{chap:PJMatrixReview}
      \input{projection/projectionWithMatrixComparison.tex}
   \chapter{Oblique projection and reciprocal frame vectors}
      \input{projection/obliqueProj.tex}
   \chapter{Projection and Moore-Penrose vector inverse}
      \input{projection/projectionAndMoorePenroseVectorInverse.tex}
   \chapter{Angle between geometric elements}
      \input{projection/angleBetweenLineAndPlane.tex}
   \chapter{Orthogonal decomposition take II}
      \input{projection/orthodecomp.tex}
   \chapter{Matrix of grade k multivector linear transformations}
      \input{projection/matrixOfLinearTx.tex}
   \chapter{Vector form of Julia fractal}
      \input{projection/juliaVector.tex}

\part{Rotation}
   \chapter{Rotor Notes}\label{chap:rotor}
      \input{rotation/rotor.tex}
   \chapter{Euler Angle Notes}\label{chap:PJEulerAngle}
      \input{rotation/eulerangle.tex}
   \chapter{Spherical polar coordinates}
      \input{rotation/sphericalPolar.tex}
   \chapter{Rotor interpolation calculation}
      \input{rotation/slerp.tex}
   \chapter{Exponential of a blade}
      \input{rotation/kvectorExponential.tex}
   \chapter{Generator of rotations in arbitrary dimensions}
      \input{rotation/rotationGenerator.tex}
   \chapter{Spherical Polar unit vectors in exponential form}
      \input{rotation/sphericalPolarUnit.tex}
   \chapter{Infinitesimal rotations}
      \input{rotation/infinitesimalRotation.tex}

\part{Calculus}
   \chapter{Developing some intuition for Multivariable and Multivector Taylor Series}\label{chap:PJmultiTaylors}
      \input{calculus/multivectorTaylors.tex}
   \chapter{Exterior derivative and chain rule components of the gradient}
      \input{calculus/gradientAndForms.tex}
   \chapter{Spherical and hyperspherical parametrization}
      \input{calculus/nvolume.tex}
   \chapter{Vector Differential Identities}
      \input{calculus/vectorDifferentialIdentities.tex}
   \chapter{Polar form for the gradient and Laplacian}
   %\chapter{Was Professor Dmitrevsky right about insisting the Laplacian is not generally \texorpdfstring{\(\spacegrad \cdot \spacegrad\)}{gradient dotted with gradient}}
      \input{calculus/polarGradAndLaplacian.tex}
   \chapter{Derivation of the spherical polar Laplacian}
      \input{calculus/sphericalPolarLaplacian.tex}
   \chapter{Tangent planes and normals in three and four dimensions}
      \input{calculus/tangentAndNormalVectors.tex}

   \chapter{Stokes' theorem}
      \input{calculus/stokesTheoremGeometricAlgebra.tex}
      %\section{Problems} % already a problems section above
         \input{calculus/stokesCorollariesGriffiths.tex}
   \chapter{Fundamental Theorem of Geometric Calculus}
      \section{Fundamental Theorem of Geometric Calculus}
         \input{calculus/fundamentalTheoremOfCalculus.tex}
         \input{calculus/fundamentalTheoremOfCalculusMaxwells.tex}
      \section{Green's function for the gradient in Euclidean spaces.}
         \input{calculus/gradientGreensFunction.tex}
         \input{calculus/gradientGreensFunctionMaxwells.tex}
      % example:
         \input{calculus/biotSavartGreens.tex}
      \section{Problems}
         % convert to example?
         \input{calculus/helmholtzDerviationMultivector.tex}
%\paragraph{Future thought.}
%
%Does this Green's function also work for mixed metric spaces?  If so, in such a metric, what does it mean to
%calculate the surface area of a unit sphere in a mixed signature space?

\part{General Physics}
   \chapter{Angular Velocity and Acceleration (Again)}\label{chap:PJAngAcc}
      \input{physics/angularAcc.tex}
   \chapter{Cross product Radial decomposition}\label{chap:PJAngAccCross}
      \input{physics/angularAccCross.tex}
   \chapter{Kinetic Energy in rotational frame}\label{chap:PJKeRot}
      \input{physics/keRotation.tex}
   \chapter{Polar velocity and acceleration}
      \input{physics/radial.tex}
   \chapter{Gradient and tensor notes}
      \input{physics/tensor.tex}
   \chapter{Inertial Tensor}
      \input{physics/inertialTensor.tex}
   \chapter{Satellite triangulation over sphere}
      \input{physics/locateSatellite.tex}
   \chapter{Exponential Solutions to Laplace Equation in \texorpdfstring{\R{N}}{ND}}
      \input{physics/laplace.tex}
   \chapter{Hyper complex numbers and symplectic structure}
      \input{physics/complex.tex}
   \chapter{Newton's method for intersection of curves in a plane}
      \input{physics/intersectionNewton.tex}
   \chapter{Center of mass of a toroidal segment}
      \input{physics/torusCenterOfMass.tex}

%\input{whatsTheDifference.tex}
